...
size_t sl_string__copy_to(const sl_string_t string, char *copy_to,
                          size_t copy_to_max_size) {
  const size_t len = sl_string__get_length(string);
  const size_t bytes_to_copy_excluding_null =
      (copy_to_max_size > 0) ? (copy_to_max_size - 1) : 0;
  const size_t bytes_to_copy =
      (len < bytes_to_copy_excluding_null) ? len : bytes_to_copy_excluding_null;
