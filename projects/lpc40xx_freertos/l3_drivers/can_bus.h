 ...
 * are stored directly into the CAN RAM (separate 2K RAM) without CPU
 * intervention.
 *
 * FullCAN feature summary:
 *      - Messages go straight into dedicated RAM, SW never needs to be
 * interrupted
 *      - Each FullCAN entry can generate an interrupt (up to 64 messages)
 *      - FullCAN entry can only be 11-bit NOT 29-bit type.
 *      - Note that FullCAN is for BOTH CANs.  If you wish to have separate
 * message reads of individual CANs, you can enable non FullCAN filters because
 * the implementation provides queues of separate CAN channels.
 *
 * When the CAN bus is initialized, we initially configure it to not accept any
 * messages until filtering or bypass option is used.  For the filtering, in
 * addition to the FullCAN, you can configure explicit 11-bit and 29-bit IDs
 * (including groups) and only these messages will be accepted.
 *
 * Note that if you send a message, and no other node acknowledges the message
 * sent, then the CAN BUS may enter "Bus off" state due to error(s).  You must
 * correct this situation and reset the CAN BUS.
 */
#pragma once
...
/**
 * Type definition of a CAN message with bit-fields (assuming little-endian
 * machine) DO NOT CHANGE THIS STRUCTURE - it maps to the hardware
 */
typedef struct {
  union {
    uint32_t
        frame; ///< 32-bit CAN frame aligned with frame_fields (bit members)
    struct {
 ...
 *
 *   can__bypass_filter_accept_all_msgs(); // If you do not add a filter, just
 * accept all messages can__reset_bus();
 * @endcode
 *
 * @param baudrate_kbps  The CAN Bus baud-rate, such as 100, 250, 500, 1000
 *                       Precise, external crystal should be used for higher
 * than 100kbps
 ...
 * @note  Both bus_off_cb and data_ovr_cb are optional.  BUS overrun callback
 * should be used because if user doesn't correct this, the CAN BUS will never
 * recover once the BUS error state is entered.
 *
 ...
 * @param timeout_ms  If FreeRTOS is running, the task will block until a
 * message arrives. Otherwise we will poll and wait this timeout to receive a
 * message.
 ...
 * if the transmit queue is full If FreeRTOS is not running the timeout is
 * simply, ignored, and false is returned if all three HW buffers are full
 *
 * @return  If CAN message was either sent or queued, true is returned
 *          false is returned if transmit queue is full and timeout occurs while
 * waiting for prior messages to be sent
 *
 * @code
 *   can__msg_t msg = {};
 *   msg.msg_id = 0x123;
...
uint16_t
can__get_tx_count(can__num_e can); ///< Number of messages written to the CAN HW
uint16_t can__get_rx_count(can__num_e can); ///< Number of messages successfully
                                            ///< queued from CAN interrupt

/**
 * Messages can be dropped out either if the receive queue is too small, or if
 * there is no consumer or task that dequeues the received messages quickly
 * enough from the can__rx() API
 */
uint16_t can__get_rx_dropped_count(can__num_e can);
/** @} */

/* ---------------------------------------------------------------------------------------
 * Rest of the API is for filtering specific messages. Any messages not defined
 * in the acceptance filter are ignored, and will not be acknowledged by the CAN
 * BUS.
 * ---------------------------------------------------------------------------------------
 */
 ...
 * disabled (disable bit set to 1) to make the count an even number. DO NOT
 * CHANGE THIS STRUCTURE - it maps to the hardware
 */
typedef union {
  struct {
...
/**
 * @{
 * Generate and return the ID used to create the standard and extended id list
 * member.  @see can__setup_filter()
 *
 * To generate a disabled slot, just pass 0xFFFF (id) as the message id, which
 * will disable the message. This is used to generate an empty slot to make an
 * even number of entries as required by the standard id filter.
 */
...
/**
 * Adds two FullCAN entries to the acceptance filter and enables the FullCAN
 * reception. The entries must be added in groups of 2.  If the second entry is
 * not needed, simply pass 0xFFFF to can__generate_standard_id() to generate a
 * disabled entry.
 *
 * @returns true if successful.
 * @note Only 11-bit IDs can use FullCAN
 *
 * @warning This must be done BEFORE setting up other filters using
 * can__setup_filter()
 * @warning CAN BUS should not be enabled to do this because the CAN Filter is
 * put to OFF mode while the entry is added.
 *
...
/**
 * Once all the FulLCAN entries are added, and CAN filters are setup, you can
 * get the pointer in memory where the actual CAN message (FullCAN entry) is
 * stored in memory.
...
/**
 * FullCAN entries may update at any time by the HW, so this method provides a
 * means to safely read the copy of the FullCAN message.
 * @param msg_copy_ptr  The same message returned from
 * can__fullcan_get_entry_ptr()
 * @param fc_msg_ptr    The copy of the message you wish to read the data to
 * @returns true if a NEW message has been captured since the last call to this
 * method. false if HW did not receive a new message
 */
bool can__fullcan_read_msg_copy(can__fullcan_msg_t *fc_msg_ptr,
                                can__fullcan_msg_t *msg_copy_ptr);
...
/**
 * Enable CAN filter for BOTH CANs; hardware doesn't allow to enable for just
 * ONE CAN controller.
 * @param std_id_list        List of 11-bit IDs to generate an ACK for (can be
 * NULL)
 * @param sid_cnt            The size of the can_std_id_t array
 *
 * @param std_group_id_list  List of 11-bit ID groups to generate an ACK for
 * (can be NULL)
 * @param sgp_cnt            The size of the can_std_grp_id_t array
 *
 * @param ext_id_list        List of 29-bit IDs to generate an ACK for (can be
 * NULL)
 * @param eid_cnt            The size of the can_ext_id_t array
 *
 * @param ext_group_id_list List of 29-bit ID groups to generate an ACK for (can
 * be NULL)
 ...
 *          The hardware carries out its search taking for granted that the IDs
 * are ordered from lowest to highest.  If filters are wrong, CAN ISR will enter
 * a loop and your board will restart (due to watchdog).
 *
 * @warning CAN BUS should not be enabled to do this because the CAN Filter is
 * put to OFF mode while the entry is added.
 *
 * @note    The CAN filter must be setup after the CAN controller(s) is
 * initialized.
 * @note    The size entries is limited to 2K bytes.  Each entry size in bytes
 * is :
 *              - can_std_id_t      : 2
 *              - can_std_grp_id_t  : 4
 *              - can_ext_id_t      : 4
 ...
 *
 * @code
 *      const can_std_id_t slist[]      = { can__generate_standard_id(can1,
 * 0x100), can__generate_standard_id(can1, 0x110),   // 2 entries
 * can__generate_standard_id(can1, 0x120), can__generate_standard_id(can1,
 * 0x130)    // 2 entries
 *                                        };
 *      const can_std_grp_id_t sglist[] = { {can__generate_standard_id(can1,
 * 0x150), can__generate_standard_id(can1, 0x200)}, // Group 1
 * {can__generate_standard_id(can2, 0x300), can__generate_standard_id(can2,
 * 0x400)}  // Group 2
 *                                        };
 *      const can_ext_id_t *elist       = NULL; // Not used, so set it to NULL
 *      const can_ext_grp_id_t eglist[] = { {can__generate_extended_id(can1,
 * 0x3500), can__generate_extended_id(can1, 0x4500)} }; // Group 1
 *
